...
var columnsAddedAfterFirstRelease = []addedColumn{
	...
	// The #new-events channel, per guild, and each event's message in it. The
	// message is edited in place while the event is ahead or underway and
	// deleted when the event's line goes to past events. Bookkeeping: written by the
	// publisher, read only by the publisher.
...
// SetNewEventsMessageID records the event's message in #new-events, or empty
// once it is deleted. updated_at is left alone: this is the publisher noting what
// it did.
