...
func TestTheManagementTableLinksTheWebPages(t *testing.T) {
	...
	link := func(*Event) string { return "[Advanced →](https://example.test/events/7)" }
	block := buildEventTableBlock(ev, nil, true, managementButtons, link)
	if first := strings.SplitN(block.text, "\n", 2)[0]; !strings.Contains(first, "Board games") || !strings.Contains(first, "[Advanced →](https://example.test/events/7)") {
