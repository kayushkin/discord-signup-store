...
func (s *Server) applyEventEdit(before *Event, patch EventPatch, actor string) (*Event, []Signup, error) {
	...
	// Made repeating: the host becomes a regular, once — MakeHostRegular
	// leaves anyone who ever was one alone.
	if after.RecurrenceRule != "" && before.RecurrenceRule == "" {
		if err := s.store.MakeHostRegular(after.ID); err != nil {
			log.Printf("[discord-signup] make host of event %d a regular: %v", after.ID, err)
		}
	}
