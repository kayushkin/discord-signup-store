...
| GET | `/api/settings` | Every environment variable the service reads, as `servicesettings` describes it: key, variable, kind, value in force and where it came from. A secret shows only whether it is set. Read-only; `PUT` is not mounted. Under `/api/` so the public vhost refuses it. |
...
| POST | `/api/sync` | Pull native events from **every** server the bot is in and post a card for any new one, then look up everyone on a live event in Discord and record the name they go by now (`renamed` in the answer). What the scheduler job calls; names no guild, so adding a server needs no change. |
...
| POST | `/events/{id}/roster/add` | **Add now**: one or more `discord_user_id` (repeated) and `list`; `regular=on` (Going, repeating events) makes them regulars too (`attending`, `maybe` or `waitlisted`). Going ignores the limit; the waitlist is refused unless the event is full, counting everyone else going, and has a waitlist. Logged as `added` by `web:<organiser id>`, and they are told by DM (a channel mention if going and DMs are closed). Refuses an id that is not a member of the event's server; the name shown comes from that lookup. |
...
| POST | `/events/{id}/message` | Message the people on the event: `body` (at most 1500 characters), `via` (`forum` posts in the event's forum thread mentioning each of them, so they are pinged; `dm` sends each a DM), and `include_waitlisted` / `include_maybe` to reach those lists as well as going. **At most 2 per event in any 10 minutes**, whoever sends them; a send that reached nobody does not count. Recorded in `event_messages` and shown in the log. |
| POST | `/events/{id}/regulars` | `discord_user_id` and `regular` (`true`/`false`). Making someone a regular puts them on the current date as going too; stopping leaves this date alone. Repeating events only. |
| POST | `/events/{id}/holds/release` | Give back the place an invite held for `discord_user_id`; the next person waiting takes it and is messaged. The invite stands. |
| POST | `/events/{id}/invite` | `reserve=hold` keeps a place for each person until they answer, while places are free — past that, the invite is not sent and the notice says so; `reserve=past_limit` keeps nothing but lets their Join past the limit. **Send invite**: DMs each `discord_user_id` (repeated for several people; the notice says what happened to each) the event with its Join and Maybe buttons. Holds nothing. Refused while signups are not open, or if they are already going or waitlisted. Recorded in `event_invites` whether or not Discord delivered it; no channel mention when DMs are closed. |
...
| GET | `/events/{id}/members?q=` | The add box's suggestions, up to 10: Discord's member search, which matches the **start** of a username, display name or nickname (listing members needs the privileged GUILD_MEMBERS intent, which this application does not have), then anyone seen in the server — on a roster, invited or a regular — whose recorded name or short name **contains** `q`, each looked up in Discord so it shows as they are now (anyone who has left is dropped, and a changed name is written back). Each with `on_roster` and `readable_name`. Editors only. |
