...
| **Management table** (`#event-management`) | organisers | The same events with **Edit**, **Repeat**, **Close/Reopen signups** (it says *waitlist* while the event is full), **End** (once it has started) and **Cancel** on each row, and **Create an event** on the end, with **Advanced Settings** under it linking the web pages; an **Advanced →** link by each event's name opens its page. Where anything about an event is changed. |
