...
func TestTheRosterTableHasNoEditButton(t *testing.T) {
	...
	if strings.Join(labels, ",") != "Join,Maybe,Leave,Details" {
		t.Errorf("buttons = %v, want Join, Maybe, Leave and Details", labels)
	}
	// Closed takes away everything that signs someone up, Maybe included.
	ev.Status = StatusClosed
	labels = labels[:0]
	for _, b := range eventTableButtons(ev) {
		labels = append(labels, b.(map[string]any)["label"].(string))
	}
	if strings.Join(labels, ",") != "Details" {
		t.Errorf("closed buttons = %v, want Details only", labels)
	}
}
...
func TestTheHeadlineLeavesOutWhatItDoesNotHave(t *testing.T) {
	...
	if got := eventTableHeadline(ev); got != "**Board Game Night**\n🗓️ Tue 9/22 5pm (weekly)\n📍 Baldini's" {
		t.Errorf("weekly = %q", got)
	}
	if got := eventSummaryLine(ev); got != "**Board Game Night** - Tue 9/22 5pm (weekly) 📍 Baldini's" {
		t.Errorf("past-events line = %q, want it kept on one line", got)
	}
}
